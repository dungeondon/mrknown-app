...
export default function HeroCarousel() {
  ...
  return (
    <div className="flex flex-col gap-2" role="region" aria-label="Featured product carousel">
      {/* Main slide */}
      <div
        className="rounded-2xl overflow-hidden relative h-56 sm:h-72 flex items-end p-6"
      ...
      >
        ...
        <div className="relative z-10 text-white">
          <p className="text-xl font-bold leading-snug whitespace-pre-line">{slide.title}</p>
          <p className="text-sm text-gray-300 mt-1">{slide.subtitle}</p>
        </div>

        {/* Dot controls */}
        <div className="absolute bottom-4 right-4 flex gap-1.5 z-10" role="tablist" aria-label="Slide indicators">
          {SLIDES.map((s, i) => (
            <button
              key={i}
              role="tab"
          ...
          return (
            <button
              ...
              className={`flex-1 rounded-lg overflow-hidden h-14 relative border-2 transition-all ${
                slideIdx === active ? 'border-orange-500' : 'border-transparent hover:border-gray-300'
              }`}
            >
              <Image
